...
| lib/providers/inventory_provider.dart:151 | medium | duplication | `_pendingPersistence` 顺序写队列模式在 inventory/shopping/custom_recipe provider 三处独立重复;命名不一(_queuePersistence vs _queueSave)。 | 提取为 mixin 或 helper class `PersistenceQueue`,三个 Notifier 复用,统一命名 `_queuePersistence`。 | HIGH | quality | approved | done |
...
| lib/screens/ingredient_detail_screen.dart:60 | low | naming | inventory_screen.dart:27 用 `_indexOfInventoryItem`,ingredient_detail_screen.dart:60 用 `_indexOf`,均封装 `inventoryIndexOf(...)`。 [blocked by HIGH item in same file] | 统一为 `_indexOfInventoryItem` 或直接调用顶层 `inventoryIndexOf`。 | LOW | quality | auto-approved | done |
...
- 2026-05-08 Batch 5 (PersistenceQueue mixin + naming 一致化) 完成: 2 个 commit。新增 `lib/providers/_persistence_queue.dart` 提取 `PersistenceQueue` mixin,统一 `queuePersistence(write)` 命名;三个 Notifier (Inventory / Shopping / CustomRecipe) 复用 mixin、删除各自的 `_pendingPersistence` 字段与 `_queuePersistence` / `_queueSave` 私有方法,CustomRecipe 的 `_mutate`(含 identity short-circuit)保留为 Notifier 私有层,内部改用 `queuePersistence(...)`。新增 `test/persistence_queue_test.dart`(serialization + failure-isolation 2 个用例)。同时清理 inventory_screen / ingredient_detail_screen 中两个 trivial wrapper(`_indexOfInventoryItem` / `_indexOf`),改为直接调用顶层 `inventoryIndexOf(...)`。轻微行为升级:mixin 在写失败时会 `debugPrint`(原实现仅 silent swallow),仍不传播,效果上对调用方等价。共 2 项 finding 标记 done(inventory:151 duplication/naming 合并行 + ingredient_detail:60 naming)。`flutter analyze` 0 issues,`flutter test` 188/188 通过(基线 186 + 新增 2 个 mixin 测试)。
