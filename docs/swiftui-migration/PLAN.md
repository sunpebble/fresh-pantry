...
- ⬜ **真正余下**(均为外部阻断,非代码):① 退役 Flutter `apps/mobile`——**2026-06-09 决定暂时保留**(`apps/mobile` 仍是 TestFlight 在发产品;同步对等已实活验证,待 `apps/ios` 充分回归/上架后再退役);② 条码相机扫描真机验证——需物理设备(模拟器降级为「请在真机使用」),硬件阻断。**SwiftUI 重写功能完整 + 同步实活验证通过 = 迁移实质达成。**
